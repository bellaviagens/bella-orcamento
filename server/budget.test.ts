...
describe("gestão de passeios e roteiro", () => {
  ...
  it("inclui uma opção gastronômica validada no dia escolhido ou nas Dicas e Links Úteis", async () => {
    ...
    const option = {
      ...
      photoUrl: "https://images.example/restaurant-1.jpg",
    };
    const baseBudget = {
      ...defaultBudgetData,
    ...
    expect(getItineraryDayActivities(withMeal.itinerary[0])[0]).toMatchObject({ kind: "meal", title: option.name, linkUrl: option.website, addressUrl: option.mapsUrl, photoUrl: option.photoUrl, importantNotes: `Local: ${option.address}` });
