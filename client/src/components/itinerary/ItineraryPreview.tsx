                ...
                <div className="flex gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className={`mt-2.5 ${photoUrl ? "grid items-start gap-3 sm:grid-cols-[138px_minmax(0,1fr)]" : ""}`}>
                      {photoUrl && <a href={photoUrl} target="_blank" rel="noreferrer" className="block self-start" aria-label={`Abrir foto de ${activity.title}`}><img src={photoUrl} alt={`Foto de ${activity.title}`} crossOrigin="anonymous" onError={(event) => event.currentTarget.parentElement?.remove()} className="h-28 w-full rounded-lg border border-slate-200 object-cover" /></a>}
