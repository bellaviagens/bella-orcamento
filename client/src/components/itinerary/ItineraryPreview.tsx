    ...
    <div id="itinerary-document" className="w-full max-w-2xl rounded-2xl bg-white p-7 text-[#1a2e4a] shadow-xl" style={{ fontFamily: "Poppins, sans-serif" }}>
      <div className="border-b-2 border-amber-400 pb-5">
        <div className="mb-2 flex items-center gap-2 text-amber-600">
          ...
          <span className="text-xs font-bold uppercase tracking-[0.18em]">Pós-aprovação</span>
        </div>
      ...
      {days.length === 0 ? (
        <div className="py-16 text-center text-sm text-slate-500">
          Adicione os dias na aba <strong>Roteiro</strong> para montar esta visualização.
        </div>
      ) : (
            ...
            return (
              <article key={day.id} className="flex gap-4">
                ...
                <div className="min-w-0 flex-1 pb-4">
                    ...
                    <div className="mt-2 rounded-lg border border-slate-200 bg-slate-50 p-3">
                      <div className="flex gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          {tour.description && <p className="mt-2 text-xs leading-relaxed text-slate-600">{tour.description}</p>}

                          <div className="mt-3 flex gap-3 text-xs font-semibold text-[#1a2e4a]">
                            {tour.pageUrl && (
                              <a href={tour.pageUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 hover:text-amber-600">
                                <ExternalLink className="h-3 w-3" />
                                Página
                              </a>
                            )}
                            {tour.photosUrl && (
                              <a href={tour.photosUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 hover:text-amber-600">
                  ...
                  {hasAdditionalNotes && <p className="mt-3 text-sm leading-relaxed text-slate-600">{day.notes}</p>}
                </div>
              </article>
            );
          })}
