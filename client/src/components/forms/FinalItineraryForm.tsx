    ...
    <div className="space-y-4">
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-3">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1"><h4 className="text-sm font-bold text-[#1a2e4a]">Adicionar informações já cadastradas</h4><p className="mt-1 text-xs text-slate-500">Após abrir o orçamento salvo e selecionar <strong>Roteiro final</strong>, os passeios aprovados podem ser copiados abaixo sem redigitar. Dia, horário, descrição, endereço, foto e alertas são aproveitados; os cadastros originais permanecem intactos.</p></div>
          <Button type="button" variant="ghost" size="sm" onClick={() => setIsRegisteredInformationCollapsed((current) => !current)} aria-expanded={!isRegisteredInformationCollapsed} className="h-11 shrink-0 min-w-24 px-3 text-xs text-slate-500 hover:bg-slate-50 hover:text-[#1a2e4a] sm:h-8 sm:min-w-0 sm:px-2"><ChevronDown className={`mr-1 h-4 w-4 transition-transform ${isRegisteredInformationCollapsed ? "" : "rotate-180"}`} />{isRegisteredInformationCollapsed ? "Abrir" : "Recolher"}</Button>
        </div>
        ...
        >
          ...
          {!isEventCollapsed && <div className="grid gap-3 sm:grid-cols-2">
            ...
            {event.kind === "hotel" && <>
              ...
              <div><Label>Bairro</Label><Input value={event.hotelNeighborhood || ""} onChange={(nativeEvent) => updateFinalItineraryEvent(event.id, { hotelNeighborhood: nativeEvent.target.value })} placeholder="Ex.: Providencia" className="mt-1 bg-white" /></div>
            ...
            {(event.kind === "hotel" || event.kind === "flight" || event.kind === "return") && <div className="sm:col-span-2 rounded-lg border border-dashed border-[#1a2e4a]/30 bg-blue-50/60 p-3">
              ...
              {(event.kind === "flight" || event.kind === "return") && (
                <div className="mt-3 rounded-md border border-blue-100 bg-white p-2.5">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <p className="inline-flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wide text-[#1a2e4a]"><Users className="h-3.5 w-3.5" />Passageiros deste voo</p>
                    <span className="text-[10px] text-slate-500">Cadastre e vincule o documento no mesmo cartão.</span>
                  </div>
                  <div className="mt-2 flex gap-2">
                    <Input value={newPassengerName} onChange={(nativeEvent) => setNewPassengerName(nativeEvent.target.value)} onKeyDown={(nativeEvent) => { if (nativeEvent.key === "Enter") { nativeEvent.preventDefault(); addPassenger(); } }} placeholder="Nome do passageiro" className="h-9 bg-slate-50 text-xs" />
                    <Button type="button" variant="outline" size="sm" onClick={addPassenger} className="h-9 shrink-0 bg-white text-xs"><Plus className="mr-1 h-3.5 w-3.5" />Adicionar</Button>
                  </div>
                  {(finalItinerary.passengers || []).length > 0 ? (
                    <div className="mt-2 space-y-1.5">
                      {finalItinerary.passengers?.map((passenger) => {
                        const passengerAttachments = (event.attachments || []).filter((attachment) => attachment.passengerId === passenger.id || attachment.passengerName?.trim().toLocaleLowerCase() === passenger.name.trim().toLocaleLowerCase());
                        const checkedIn = passengerAttachments.length > 0;
                        return (
                          <div key={passenger.id} className="rounded border border-slate-100 bg-slate-50 px-2 py-1.5">
                            <div className="flex flex-wrap items-center justify-between gap-2">
                              <div className="flex min-w-0 items-center gap-1.5"><UserRound className="h-3.5 w-3.5 shrink-0 text-[#1a2e4a]" /><span className="truncate text-xs font-semibold text-[#1a2e4a]">{passenger.name}</span><span className={`rounded-full px-1.5 py-0.5 text-[10px] font-bold ${checkedIn ? "bg-emerald-600 text-white" : "bg-slate-200 text-slate-500"}`}>{checkedIn ? "Documento anexado" : "Aguardando documento"}</span></div>
                              <button type="button" onClick={() => removePassenger(passenger.id)} className="text-[11px] text-slate-400 hover:text-red-600" title={`Remover ${passenger.name}`}>Remover</button>
                            </div>
                            {passengerAttachments.length > 0 ? (
                              <div className="mt-1.5 space-y-1">
                                {passengerAttachments.map((attachment) => <a key={attachment.id} href={attachment.url} target="_blank" rel="noreferrer" className="flex min-w-0 items-center gap-1.5 text-[11px] font-medium text-[#1a2e4a] hover:text-amber-700 hover:underline"><FileText className="h-3.5 w-3.5 shrink-0" /><span className="truncate">{attachment.name}</span>{attachment.seat ? <span className="shrink-0 rounded bg-amber-50 px-1 py-0.5 text-[10px] text-amber-800">Assento {attachment.seat}</span> : null}</a>)}
                              </div>
                            ) : null}
                          </div>
                        );
                  ...
                  ) : <p className="mt-2 text-xs text-slate-500">Cadastre o passageiro antes de anexar o cartão ou bilhete.</p>}
                </div>
              )}
              {(event.kind === "flight" || event.kind === "return") && <div className="mt-3 rounded-md border border-amber-200 bg-amber-50/60 p-2.5"><Label htmlFor={`final-baggage-rules-${event.id}`} className="inline-flex items-center gap-1.5 text-[11px] font-bold text-[#1a2e4a]"><Luggage className="h-3.5 w-3.5" />Regras de bagagem da companhia aérea</Label><Input id={`final-baggage-rules-${event.id}`} value={finalItinerary.baggageRulesUrl || ""} onChange={(nativeEvent) => updateFinalItinerary({ enabled: true, baggageRulesUrl: nativeEvent.target.value })} placeholder="Cole aqui o link da companhia para itens permitidos e franquia de bagagem" className="mt-1.5 h-9 bg-white text-xs" />{finalItinerary.baggageRulesUrl && <a href={finalItinerary.baggageRulesUrl} target="_blank" rel="noreferrer" className="mt-1.5 inline-flex items-center gap-1 text-[11px] font-semibold text-[#1a2e4a] hover:text-amber-700 hover:underline"><ExternalLink className="h-3.5 w-3.5" />Abrir regras da companhia</a>}</div>}
