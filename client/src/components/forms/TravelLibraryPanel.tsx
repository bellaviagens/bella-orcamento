...
export function TravelLibraryPanel({ initiallyOpen = false, restaurantEditId, onRestaurantEditOpened }: { initiallyOpen?: boolean; restaurantEditId?: string | null; onRestaurantEditOpened?: () => void }) {
    ...
    : selectedCategory === "tour"
      ...
      : selectedCategory === "transfer"
        ...
        : selectedCategory === "restaurant"
          ? "Buscar restaurante por nome, destino ou contato"
          : "Buscar por nome, destino, bairro ou contato";
  ...
  return <section ref={panelRef} className="rounded-lg border border-[#1a2e4a]/15 bg-blue-50/60 p-3">
    <div className="flex flex-wrap items-center justify-between gap-2">
      ...
      <div className="flex items-center gap-1.5"><Button type="button" variant="outline" size="sm" onClick={openNewItem} className="h-9 bg-white px-3 text-xs font-semibold"><Plus className="mr-1 h-4 w-4" />Adicionar</Button><Button type="button" variant="ghost" size="sm" onClick={() => setIsOpen((current) => !current)} aria-expanded={isOpen} className="h-9 min-w-24 px-3 text-xs text-slate-600 hover:bg-white hover:text-[#1a2e4a]"><ChevronDown className={`mr-1.5 h-4 w-4 transition-transform ${isOpen ? "rotate-180" : ""}`} />{isOpen ? "Recolher" : "Abrir"}</Button></div>
    </div>
    {isOpen && <div className="mt-3 space-y-3 border-t border-[#1a2e4a]/10 pt-3">
      ...
      <div className="flex flex-wrap items-center gap-1.5"><span className="mr-1 text-xs font-semibold text-[#1a2e4a]">Exibir:</span><Button type="button" variant={selectedSource === "library" ? "default" : "outline"} size="sm" onClick={() => setSelectedSource("library")} className={selectedSource === "library" ? "h-8 bg-[#1a2e4a] px-2 text-xs text-white hover:bg-[#233f67]" : "h-8 bg-white px-2 text-xs text-[#1a2e4a]"}>Itens salvos</Button><Button type="button" variant={selectedSource === "favorites" ? "default" : "outline"} size="sm" onClick={() => setSelectedSource("favorites")} className={selectedSource === "favorites" ? "h-8 bg-[#1a2e4a] px-2 text-xs text-white hover:bg-[#233f67]" : "h-8 bg-white px-2 text-xs text-[#1a2e4a]"}><Heart className="mr-1 h-3.5 w-3.5" />Favoritos</Button>{selectedSource === "library" && <><div className="hidden h-5 w-px bg-slate-200 sm:block" /><div className="hidden flex-wrap items-center gap-1.5 sm:flex">{categoryControls}</div><Button type="button" variant="outline" size="sm" onClick={clearAllFilters} className="h-8 bg-white px-2 text-xs text-slate-600 sm:ml-auto"><RotateCcw className="mr-1 h-3.5 w-3.5" />Limpar filtros</Button></>}</div>
	      {selectedSource === "library" ? <><div className="mt-2 space-y-2"><div className="flex flex-wrap gap-1.5 sm:hidden">{categoryControls}</div><div className={selectedCategory === "hotel" ? "grid gap-1.5 sm:grid-cols-[minmax(0,1fr)_190px]" : "grid gap-1.5"}><div className="relative"><Search className="pointer-events-none absolute left-2 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-slate-400" /><Input value={hotelSearchQuery} onChange={(event) => setHotelSearchQuery(event.target.value)} placeholder={searchPlaceholder} aria-label="Buscar na Biblioteca de Viagem" className="h-8 bg-white pl-7 text-xs" /></div>{selectedCategory === "hotel" && <Select value={hotelSort} onValueChange={(value: HotelLibrarySort) => setHotelSort(value)}><SelectTrigger className="h-8 bg-white text-xs"><SelectValue placeholder="Ordenar hotéis" /></SelectTrigger><SelectContent><SelectItem value="default">Ordem padrão</SelectItem><SelectItem value="neighborhood">Ordenar por bairro</SelectItem></SelectContent></Select>}</div><div className="grid gap-1.5 sm:grid-cols-3"><Select value={selectedCountry || "all"} onValueChange={(value) => { setSelectedCountry(value === "all" ? "" : value); setSelectedCity(""); setSelectedNeighborhood(""); }}><SelectTrigger className="h-8 bg-white text-xs"><SelectValue placeholder="Todos os países" /></SelectTrigger><SelectContent><SelectItem value="all">Todos os países</SelectItem>{countries.map((country) => <SelectItem key={country} value={country}>{country}</SelectItem>)}</SelectContent></Select><Select value={selectedCity || "all"} onValueChange={(value) => { setSelectedCity(value === "all" ? "" : value); setSelectedNeighborhood(""); }}><SelectTrigger className="h-8 bg-white text-xs"><SelectValue placeholder="Todas as cidades" /></SelectTrigger><SelectContent><SelectItem value="all">Todas as cidades</SelectItem>{cities.map((city) => <SelectItem key={city} value={city}>{city}</SelectItem>)}</SelectContent></Select><Select value={selectedNeighborhood || "all"} onValueChange={(value) => setSelectedNeighborhood(value === "all" ? "" : value)}><SelectTrigger className="h-8 bg-white text-xs"><SelectValue placeholder="Todos os bairros" /></SelectTrigger><SelectContent><SelectItem value="all">Todos os bairros</SelectItem>{neighborhoods.map((neighborhood) => <SelectItem key={neighborhood} value={neighborhood}>{neighborhood}</SelectItem>)}</SelectContent></Select></div></div>
