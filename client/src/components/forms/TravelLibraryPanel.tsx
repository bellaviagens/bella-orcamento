...
import { Building2, BusFront, ChevronDown, FileText, FolderOpen, Hotel, ImagePlus, Pencil, Plus, RotateCcw, Search, SendHorizontal, Trash2, Utensils } from "lucide-react";
...
export function TravelLibraryPanel({ initiallyOpen = false }: { initiallyOpen?: boolean }) {
  ...
  const clearAllFilters = () => { setSelectedCategory("all"); setSelectedCountry(""); setSelectedCity(""); setSelectedNeighborhood(""); setHotelSearchQuery(""); setHotelSort("default"); };
  const openNewItem = () => {
  ...
  return <section className="rounded-lg border border-[#1a2e4a]/15 bg-blue-50/60 p-3">
    ...
    {isOpen && <div className="mt-3 space-y-3 border-t border-[#1a2e4a]/10 pt-3">
      ...
      <div className="flex justify-end"><Button type="button" variant="outline" size="sm" onClick={clearAllFilters} className="h-8 bg-white px-2 text-xs text-slate-600"><RotateCcw className="mr-1 h-3.5 w-3.5" />Limpar filtros</Button></div>
