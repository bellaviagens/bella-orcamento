...
import { Building2, BusFront, ChevronDown, FileText, FolderOpen, Hotel, ImagePlus, Pencil, Plus, Search, SendHorizontal, Trash2, Utensils } from "lucide-react";
...
import { filterTravelLibraryItems, getTravelLibraryFolders, sortTravelLibraryItems, TRAVEL_LIBRARY_CATEGORY_LABELS, type HotelLibrarySort, type TravelLibraryCategory } from "./travelLibraryState";
...
export function TravelLibraryPanel({ initiallyOpen = false }: { initiallyOpen?: boolean }) {
  ...
  const neighborhoods = useMemo(() => Array.from(new Set(allItems.filter((item) => item.category === "hotel" && (!selectedCountry || item.country?.trim() === selectedCountry) && (!selectedCity || item.city?.trim() === selectedCity)).map((item) => item.neighborhood?.trim()).filter((value): value is string => Boolean(value)))).sort((a, b) => a.localeCompare(b, "pt-BR")), [allItems, selectedCountry, selectedCity]);
  ...
  return <section className="rounded-lg border border-[#1a2e4a]/15 bg-blue-50/60 p-3">
    ...
    {isOpen && <div className="mt-3 space-y-3 border-t border-[#1a2e4a]/10 pt-3">
      ...
      <div className="space-y-1.5"><div className="grid gap-1.5 sm:grid-cols-[minmax(0,1fr)_150px_150px_150px]"><div className="flex flex-wrap gap-1.5"><Button type="button" variant={selectedCategory === "all" ? "default" : "outline"} size="sm" onClick={() => { setSelectedCategory("all"); setSelectedNeighborhood(""); setHotelSearchQuery(""); setHotelSort("default"); }} className="h-8 bg-white px-2 text-xs text-[#1a2e4a]">Todos</Button>{(Object.keys(TRAVEL_LIBRARY_CATEGORY_LABELS) as TravelLibraryCategory[]).map((category) => <Button key={category} type="button" variant={selectedCategory === category ? "default" : "outline"} size="sm" onClick={() => { setSelectedCategory(category); if (category !== "hotel") { setSelectedNeighborhood(""); setHotelSearchQuery(""); setHotelSort("default"); } }} className="h-8 bg-white px-2 text-xs text-[#1a2e4a]">{TRAVEL_LIBRARY_CATEGORY_LABELS[category]}</Button>)}</div><Select value={selectedCountry || "all"} onValueChange={(value) => { setSelectedCountry(value === "all" ? "" : value); setSelectedCity(""); setSelectedNeighborhood(""); }}><SelectTrigger className="h-8 bg-white text-xs"><SelectValue placeholder="Todos os países" /></SelectTrigger><SelectContent><SelectItem value="all">Todos os países</SelectItem>{countries.map((country) => <SelectItem key={country} value={country}>{country}</SelectItem>)}</SelectContent></Select><Select value={selectedCity || "all"} onValueChange={(value) => { setSelectedCity(value === "all" ? "" : value); setSelectedNeighborhood(""); }}><SelectTrigger className="h-8 bg-white text-xs"><SelectValue placeholder="Todas as cidades" /></SelectTrigger><SelectContent><SelectItem value="all">Todas as cidades</SelectItem>{cities.map((city) => <SelectItem key={city} value={city}>{city}</SelectItem>)}</SelectContent></Select><Select value={selectedNeighborhood || "all"} onValueChange={(value) => setSelectedNeighborhood(value === "all" ? "" : value)} disabled={selectedCategory !== "hotel" && !selectedNeighborhood}><SelectTrigger className="h-8 bg-white text-xs"><SelectValue placeholder="Todos os bairros" /></SelectTrigger><SelectContent><SelectItem value="all">Todos os bairros</SelectItem>{neighborhoods.map((neighborhood) => <SelectItem key={neighborhood} value={neighborhood}>{neighborhood}</SelectItem>)}</SelectContent></Select></div>{selectedCategory === "hotel" && <div className="grid gap-1.5 sm:grid-cols-[minmax(0,1fr)_190px]"><div className="relative"><Search className="pointer-events-none absolute left-2 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-slate-400" /><Input value={hotelSearchQuery} onChange={(event) => setHotelSearchQuery(event.target.value)} placeholder="Buscar hotel por nome, endereço ou bairro" className="h-8 bg-white pl-7 text-xs" /></div><Select value={hotelSort} onValueChange={(value: HotelLibrarySort) => setHotelSort(value)}><SelectTrigger className="h-8 bg-white text-xs"><SelectValue placeholder="Ordenar hotéis" /></SelectTrigger><SelectContent><SelectItem value="default">Ordem padrão</SelectItem><SelectItem value="neighborhood">Ordenar por bairro</SelectItem></SelectContent></Select></div>}</div>
  {libraryQuery.isLoading ? <p className="text-xs text-slate-500">Carregando biblioteca...</p> : destinations.length ? <div className="space-y-3">{destinations.map((destination) => { const destinationItems = items.filter((item) => (item.destination?.trim() || "Sem destino definido") === destination); const folders = getTravelLibraryFolders(destinationItems); return <div key={destination} className="rounded-md border border-blue-100 bg-white p-2.5"><div className="mb-2 flex items-center gap-1.5 text-xs font-bold text-[#1a2e4a]"><FolderOpen className="h-3.5 w-3.5" />{destination}</div><div className="space-y-2">{folders.map((folder) => <div key={folder} className="rounded border border-slate-100 bg-slate-50 p-2"><p className="mb-1 text-[11px] font-bold uppercase tracking-wide text-slate-500">{folder}</p><div className="space-y-1.5">{destinationItems.filter((item) => item.folderName === folder).map((item) => { const Icon = CATEGORY_ICONS[item.category]; const contact = item.category === "transfer" ? [item.responsibleName || item.contactName, item.whatsapp || item.phone].filter(Boolean).join(" • ") : [item.contactName, item.phone].filter(Boolean).join(" • "); const hotelLocation = item.category === "hotel" ? [item.neighborhood, item.city, item.country].filter(Boolean).join(", ") : [item.city, item.country].filter(Boolean).join(", "); return <div key={item.id} className="flex flex-wrap items-center justify-between gap-2 rounded border border-slate-100 bg-white px-2 py-1.5"><div className="flex min-w-0 items-center gap-2">{item.imageUrl ? <img src={item.imageUrl} alt="" className="h-9 w-9 rounded object-cover" /> : <Icon className="h-3.5 w-3.5 shrink-0 text-[#1a2e4a]" />}<div className="min-w-0"><div className="flex items-center gap-1.5"><span className="text-xs font-semibold text-[#1a2e4a]">{item.name}</span><span className="text-[10px] text-slate-400">{TRAVEL_LIBRARY_CATEGORY_LABELS[item.category]}</span></div>{hotelLocation && <p className="mt-0.5 text-[10px] text-slate-400">{hotelLocation}</p>}{contact && <p className="mt-0.5 text-[11px] text-slate-500">{contact}</p>}</div></div><div className="flex flex-wrap justify-end gap-1"><Button type="button" variant="outline" size="sm" onClick={() => openExistingItem(item)} className="h-7 bg-white px-2 text-[11px]"><Pencil className="mr-1 h-3 w-3" />Editar</Button><Button type="button" variant="outline" size="sm" onClick={() => importIntoBudget(item)} className="h-7 bg-white px-2 text-[11px]"><SendHorizontal className="mr-1 h-3 w-3" />Importar</Button>{item.linkUrl && <Button type="button" variant="outline" size="sm" asChild className="h-7 bg-white px-2 text-[11px]"><a href={item.linkUrl} target="_blank" rel="noreferrer">Abrir</a></Button>}{item.documentUrl && <Button type="button" variant="outline" size="sm" asChild className="h-7 w-7 bg-white p-0" aria-label={`Abrir documento de ${item.name}`}><a href={item.documentUrl} target="_blank" rel="noreferrer"><FileText className="h-3.5 w-3.5" /></a></Button>}<Button type="button" variant="ghost" size="sm" onClick={() => void remove(item.id, item.name)} disabled={deleteMutation.isPending} className="h-7 w-7 p-0 text-red-500 hover:bg-red-50 hover:text-red-700" aria-label={`Excluir ${item.name}`}><Trash2 className="h-3.5 w-3.5" /></Button></div></div>; })}</div></div>)}</div></div>; })}</div> : <p className="rounded-md border border-dashed border-blue-100 bg-white px-3 py-3 text-xs text-slate-500">Nenhum item corresponde aos filtros atuais. Crie destinos, subgrupos e fornecedores para reutilizá-los em novos orçamentos.</p>}
    </div>}
  </section>;
}
