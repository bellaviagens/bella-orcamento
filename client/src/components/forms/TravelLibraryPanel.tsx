...
import { Building2, BusFront, ChevronDown, FileText, FolderOpen, Hotel, ImagePlus, Plus, SendHorizontal, Trash2, Utensils } from "lucide-react";
...
const EMPTY_DRAFT = { category: "hotel" as TravelLibraryCategory, folderName: "", name: "", destination: "", country: "", city: "", contactName: "", phone: "", responsibleName: "", whatsapp: "", linkUrl: "", imageUrl: "", documentUrl: "", notes: "" };
...
export function TravelLibraryPanel({ initiallyOpen = false }: { initiallyOpen?: boolean }) {
  ...
  const save = async () => {
    ...
    try {
      await createMutation.mutateAsync({
        ...
        responsibleName: draft.responsibleName.trim() || undefined,
  ...
  return <section className="rounded-lg border border-[#1a2e4a]/15 bg-blue-50/60 p-3">
    <div className="flex flex-wrap items-center justify-between gap-2"><div className="flex items-center gap-2"><FolderOpen className="h-4 w-4 text-[#1a2e4a]" /><div><p className="text-sm font-bold text-[#1a2e4a]">Biblioteca de Viagem</p><p className="text-xs text-slate-500">Organize fornecedores por destino e reutilize-os no orçamento atual.</p></div></div><div className="flex items-center gap-1"><Button type="button" variant="outline" size="sm" onClick={() => { setShowForm((current) => !current); setIsOpen(true); }} className="h-10 bg-white px-3 text-xs font-semibold"><Plus className="mr-1 h-4 w-4" />Adicionar</Button><Button type="button" variant="ghost" size="sm" onClick={() => setIsOpen((current) => !current)} aria-expanded={isOpen} className="h-10 min-w-24 px-3 text-xs text-slate-600 hover:bg-white hover:text-[#1a2e4a]"><ChevronDown className={`mr-1.5 h-4 w-4 transition-transform ${isOpen ? "rotate-180" : ""}`} />{isOpen ? "Recolher" : "Abrir"}</Button></div></div>
    {isOpen && <div className="mt-3 space-y-3 border-t border-[#1a2e4a]/10 pt-3">
      {showForm && <div className="rounded-md border border-blue-100 bg-white p-3"><div className="mb-2 flex items-center justify-between"><p className="text-xs font-bold text-[#1a2e4a]">Novo item reutilizável</p><Button type="button" variant="ghost" size="sm" onClick={() => setShowForm(false)} className="h-8 px-2 text-xs">Cancelar</Button></div><div className="grid gap-2 sm:grid-cols-2"><div><Label>Tipo de serviço</Label><Select value={draft.category} onValueChange={(category: TravelLibraryCategory) => setDraft((current) => ({ ...current, category }))}><SelectTrigger className="mt-1 h-9 bg-slate-50 text-xs"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="hotel">Hotel</SelectItem><SelectItem value="tour">Passeio</SelectItem><SelectItem value="restaurant">Restaurante</SelectItem><SelectItem value="transfer">Transfer</SelectItem></SelectContent></Select></div><div><Label>Destino</Label><Input value={draft.destination} onChange={(event) => setDraft((current) => ({ ...current, destination: event.target.value }))} placeholder="Ex.: Santiago" className="mt-1 h-9 bg-slate-50 text-xs" /></div><div><Label>País</Label><Input value={draft.country} onChange={(event) => setDraft((current) => ({ ...current, country: event.target.value }))} placeholder="Ex.: Chile" className="mt-1 h-9 bg-slate-50 text-xs" /></div><div><Label>Cidade</Label><Input value={draft.city} onChange={(event) => setDraft((current) => ({ ...current, city: event.target.value }))} placeholder="Ex.: Santiago" className="mt-1 h-9 bg-slate-50 text-xs" /></div><div><Label>Subgrupo</Label><Input value={draft.folderName} onChange={(event) => setDraft((current) => ({ ...current, folderName: event.target.value }))} placeholder="Ex.: Hotéis centro" className="mt-1 h-9 bg-slate-50 text-xs" /></div><div><Label>Nome</Label><Input value={draft.name} onChange={(event) => setDraft((current) => ({ ...current, name: event.target.value }))} placeholder="Ex.: Hotel Plaza Santiago" className="mt-1 h-9 bg-slate-50 text-xs" /></div><div><Label>Contato / empresa</Label><Input value={draft.contactName} onChange={(event) => setDraft((current) => ({ ...current, contactName: event.target.value }))} placeholder="Ex.: Transfer Andes" className="mt-1 h-9 bg-slate-50 text-xs" /></div><div><Label>Telefone</Label><Input value={draft.phone} onChange={(event) => setDraft((current) => ({ ...current, phone: event.target.value }))} placeholder="Ex.: +56 9 9999-9999" className="mt-1 h-9 bg-slate-50 text-xs" /></div>{draft.category === "transfer" && <><div><Label>Nome do responsável</Label><Input value={draft.responsibleName} onChange={(event) => setDraft((current) => ({ ...current, responsibleName: event.target.value }))} placeholder="Ex.: Carlos Silva" className="mt-1 h-9 bg-slate-50 text-xs" /></div><div><Label>WhatsApp do responsável</Label><Input value={draft.whatsapp} onChange={(event) => setDraft((current) => ({ ...current, whatsapp: event.target.value }))} placeholder="Ex.: +56 9 9999-9999" className="mt-1 h-9 bg-slate-50 text-xs" /></div></>}<div><Label>Link</Label><Input value={draft.linkUrl} onChange={(event) => setDraft((current) => ({ ...current, linkUrl: event.target.value }))} placeholder="https://..." className="mt-1 h-9 bg-slate-50 text-xs" /></div><div><Label>Foto / álbum (link)</Label><Input value={draft.imageUrl} onChange={(event) => setDraft((current) => ({ ...current, imageUrl: event.target.value }))} placeholder="https://..." className="mt-1 h-9 bg-slate-50 text-xs" /></div><div className="rounded border border-dashed border-blue-100 bg-blue-50/40 p-2"><Label className="text-xs">Anexar foto</Label><Input type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => void uploadFile(event, "imageUrl")} disabled={uploadMutation.isPending} className="mt-1 h-8 bg-white text-[11px]" />{draft.imageUrl && <p className="mt-1 text-[10px] text-emerald-700">Foto pronta para salvar.</p>}</div><div className="rounded border border-dashed border-blue-100 bg-blue-50/40 p-2"><Label className="text-xs">Anexar documento / voucher</Label><Input type="file" accept="application/pdf,image/jpeg,image/png,image/webp" onChange={(event) => void uploadFile(event, "documentUrl")} disabled={uploadMutation.isPending} className="mt-1 h-8 bg-white text-[11px]" />{draft.documentUrl && <p className="mt-1 text-[10px] text-emerald-700">Documento pronto para salvar.</p>}</div><div className="sm:col-span-2"><Label>Notas</Label><Textarea value={draft.notes} onChange={(event) => setDraft((current) => ({ ...current, notes: event.target.value }))} placeholder="Informações úteis para futuros orçamentos." className="mt-1 min-h-16 bg-slate-50 text-xs" /></div></div><div className="mt-2 flex justify-end"><Button type="button" size="sm" onClick={() => void save()} disabled={createMutation.isPending || uploadMutation.isPending} className="h-9 bg-[#1a2e4a] px-3 text-xs font-bold text-white hover:bg-[#233f67]">Salvar na biblioteca</Button></div></div>}
